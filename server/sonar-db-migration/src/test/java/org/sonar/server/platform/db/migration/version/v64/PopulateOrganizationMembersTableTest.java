...
public class PopulateOrganizationMembersTableTest {
  ...
  private PopulateOrganizationMembersTable underTest = new PopulateOrganizationMembersTable(db.database(), UuidFactoryFast.getInstance(), new DefaultOrganizationUuidImpl());
  ...
  public void migrate_active_users_to_default_organization() throws Exception {
    setupDefaultOrganization();
    int user1Id = insertUser(USER1_LOGIN, true);
    int user2Id = insertUser(USER2_LOGIN, true);
    int user3Id = insertUser(USER3_LOGIN, false);

    underTest.execute();

    verifyUserMembership(user1Id, DEFAULT_ORGANIZATION_UUID);
    verifyUserMembership(user2Id, DEFAULT_ORGANIZATION_UUID);
  ...
  private void insertOrganizationMember(int userId, String organizationUuid) {
    db.executeInsert(TABLE, "UUID", Uuids.createFast(), "USER_ID", userId, "ORGANIZATION_UUID", organizationUuid);
  }
