...
public class PopulateOrganizationMembersTable extends DataChange {
  ...
  private static final String INSERT_ORGANIZATION_MEMBERS_SQL = "INSERT INTO organization_members (uuid, user_id, organization_uuid) VALUES (?, ?, ?)";

  private final UuidFactory uuidFactory;
  private final DefaultOrganizationUuid defaultOrganizationUuid;

  public PopulateOrganizationMembersTable(Database db, UuidFactory uuidFactory, DefaultOrganizationUuid defaultOrganizationUuid) {
    super(db);
  ...
  private void associateUsersToDefaultOrganization(Context context) throws SQLException {
    ...
    massUpdate.execute((row, update) -> {
      ...
      update.setInt(2, row.getInt(1));
      update.setString(3, organizationUuid);
      return true;
    });
  }

  private void associateUsersToOrganizationBasedOnPermission(Context context) throws SQLException {
    MassUpdate massUpdate = context.prepareMassUpdate().rowPluralName("organization members");
    massUpdate.select(
